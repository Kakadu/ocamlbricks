...
let register_unit_option (x:string) ?(doc="undocumented") ?(default=None) () =
  let doc = properly_append_arg_name_in_help_and_doc ~arg_name_in_help:"" ~doc in
  let result = ref default in
  let spec = (("-"^x), Arg.Unit (fun v -> result := Some v), doc) in
  (options_register := spec::!options_register);
  result
;;

let register_bool_option (x:string) ?(arg_name_in_help="BOOL") ?(doc="undocumented") ?(default=None) () =
  let doc = properly_append_arg_name_in_help_and_doc ~arg_name_in_help ~doc in
  let result = ref default in
...
let register_string_option (x:string) ?tests ?(arg_name_in_help="STRING") ?(doc="undocumented") ?(default=None) () =
  let result = ref default in
  let doc = properly_append_arg_name_in_help_and_doc ~arg_name_in_help ~doc in
...
let register_filename_option 
  ...
  register_string_option y ~arg_name_in_help ~tests ?doc ?default ()
;;

let register_directory_option (y:string) ?r ?w ?x ?tests ?(arg_name_in_help="DIR") ?doc ?default () = 
  let file_test =
...
let register_enum_option (x:string) ~admissible_args ?(doc="undocumented") ?(default=None) () =
  let result = ref default in
  let doc = properly_append_arg_name_in_help_and_doc ~arg_name_in_help:"" ~doc in
...
let register_int_option (x:string) ?tests ?(arg_name_in_help="INT") ?(doc="undocumented") ?(default=None) () =
  let result = ref default in
  let doc = properly_append_arg_name_in_help_and_doc ~arg_name_in_help ~doc in
  let callback = properly_compose_with_tests ?tests (fun v -> result := Some v) in
...
let register_float_option (x:string) ?tests ?(arg_name_in_help="FLOAT") ?(doc="undocumented") ?(default=None) () =
  let result = ref default in
  let doc = properly_append_arg_name_in_help_and_doc ~arg_name_in_help ~doc in
  let callback = properly_compose_with_tests ?tests (fun v -> result := Some v) in
